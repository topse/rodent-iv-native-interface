...
void POS::SetPosition(const char *epd) {
    ...
    if (*epd == '-')
        ...
        Glob.Castle_W_K = (eSquare) KingSq(WC);
        Glob.Castle_B_K = (eSquare) KingSq(BC);
        ...
        for (int i = 4; i > 0; i -= 1) {
            ...
            if ((*epd >= 'A') && (*epd <= 'H')) {
                ...
                if (Castle_R > Glob.Castle_W_K) {
                    Glob.Castle_W_RK = Castle_R;
                    mCFlags |= W_KS;
                } else {
                    Glob.Castle_W_RQ = Castle_R;
                    mCFlags |= W_QS;
                }
                epd++;
            }
            ...
            if ((*epd >= 'a') && (*epd <= 'h')) {
                ...
                if (Castle_R > Glob.Castle_B_K) {
                    Glob.Castle_B_RK = Castle_R;
                    mCFlags |= B_KS;
                } else {
                    Glob.Castle_B_RQ = Castle_R;
                    mCFlags |= B_QS;
                }
                epd++;
            }
            ...
            if (*epd == 'q') {
                ...
                for (i=7 ; i >= 0 ; i--)
                    if (Rooks(BC) & mClBb[BC] & SqBb(56 + i))
                        Glob.Castle_B_RQ = (eSquare) (56 + i);
            }
        }

        // "MoveToStr" did not know which side to move, so correct them also for the unneeded color
        // Only need Glob.Castle_W_RQ and Glob.Castle_W_RK, but make proper things:

        if (!(mCFlags & W_KS))
            Glob.Castle_W_RK = (eSquare) ( 0 + File(Glob.Castle_B_RK));
        if (!(mCFlags & W_QS))
            Glob.Castle_W_RQ = (eSquare) ( 0 + File(Glob.Castle_B_RQ));
        if (!(mCFlags & B_KS))
            Glob.Castle_B_RK = (eSquare) (56 + File(Glob.Castle_W_RK));
        if (!(mCFlags & B_QS))
            Glob.Castle_B_RQ = (eSquare) (56 + File(Glob.Castle_W_RQ));

        // and for completeness:
        if (!(mCFlags & (W_KS | W_QS)))
            Glob.Castle_W_K = (eSquare) ( 0 + File(Glob.Castle_B_K));
        if (!(mCFlags & (B_KS | B_QS)))
            Glob.Castle_B_K = (eSquare) (56 + File(Glob.Castle_W_K));
